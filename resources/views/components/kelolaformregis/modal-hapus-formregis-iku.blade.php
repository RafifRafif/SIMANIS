...
<div class="modal fade" id="hapusIkuModal" tabindex="-1" aria-labelledby="hapusIkuLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content text-center border-0 shadow-sm rounded-4">
      <div class="modal-body py-4">
        ...
        <form id="deleteIkuForm" method="POST">
          @csrf
          @method('DELETE')
...
<script>
  document.addEventListener('DOMContentLoaded', () => {
    document.querySelectorAll('.delete-iku-button').forEach(button => {
      button.addEventListener('click', function() {
        const id = this.dataset.id;
        document.getElementById('deleteIkuForm').action = `/iku/delete/${id}`;
      });
    });
  });
</script>